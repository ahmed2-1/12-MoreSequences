...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'DRAW_POINTS_ON_CIRCLE, test 1: 30 magenta dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(225, 150), 50)
...
def run_test_pizza():
    ...
    pizza(window, circle, 50, 'black', 0)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    for i in range(len(list_of_points)):
        pizza_cutter = rg.Line(circle.center, list_of_points[i])
        pizza_cutter.color = color
        pizza_cutter.thickness = thickness
        pizza_cutter.attach_to(window)
    window.render()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    circle.attach_to(window)
    for i in range(len(list_of_points)):
        line = rg.Line(list_of_points[i-1], list_of_points[i])
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 51, 25, 'orange', 5)
    window.close_on_mouse_click()
    ...
    fancy_polygon(window, circle, 300, 61, 'green', 5)
    window.close_on_mouse_click()
